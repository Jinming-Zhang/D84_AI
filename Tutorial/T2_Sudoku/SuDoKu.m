...
function [sol]=SuDoKu(input)
    ...
    % [iii,jjj]=ind2sub(size(input),node)
    ...
    for l=1:length(available_assignments)
        ...
        intervenes(l) = 0;
        [i,j]=ind2sub(size(input),node);
        % top neighbor
        lst = helpppp(input, i-1, j-1);
        if(length(lst)>0)
        	if(sum(lst==ass) > 0)
        		intervenes(l) = intervenes(l) + 1;
        	end;
        end;

		lst = helpppp(input, i-1, j);
        if(length(lst)>0)
        	if(sum(lst==ass) > 0)
        		intervenes(l) = intervenes(l) + 1;
        	end;
    ...
    % sort assignments based on each values intervene number
    if(length(available_assignments) > 1)
    	swap_no = 1;
    	while(swap_no > 0)
    		swap_no = 0;
    		for s=2:length(intervenes)
    			if(intervenes(s-1) > intervenes(s))
    				% swap intervenes
    				tmp = intervenes(s-1);
    				intervenes(s-1) = intervenes(s);
    				intervenes(s) = tmp;
    				% swap available_assignments
    				tmp = available_assignments(s-1);
    				available_assignments(s-1) = available_assignments(s);
    				available_assignments(s) = tmp;
    				swap_no = swap_no + 1;
    			end;
    		end;
    	end;
...
function [domain_values] = helpppp(input, i, j)
	if(i<1 || i>length(input(:,1)) || j < 1 || j > length(input(1,:)) || input(i,j)!=0)
		domain_values = [];
		return;
	end;
    ii=floor((i-1)/3);                  % Figure out which 3x3 sub-grid it's in
    ...
    % And then determine which values are left that we have to try
    domain_values=unique(setdiff([1 2 3 4 5 6 7 8 9],taken));
return;