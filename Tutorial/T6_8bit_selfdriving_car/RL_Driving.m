...
DEBUG=0;
Policy=2*ones(6250,1);            %  Initially the car will stick to its position.
Q_table=zeros(6250,3);        %  Your Q table, 6250 states, 3 actions
...
for round=1:N                 % For the expected number of training rounds
    fflush(stdout);
    disp(round)
    for s_i=1:6250 % for each state
      [i1,i2,i3,i4,i5]=indexesFromState(s_i-1); % parameters of current state
      if (i1 == -1 || i2 == -1 || i3 == -1 || i4 == -1 || i5 == -1)
        continue;
      end;

      % action of moving left
      action = i2;
      if(i2 > 0) % when there is sapce to move, else stay at left-most place
        action = i2 - 1;
      end;
      r_state = i1 + (action*5) + (i3*50) + (i4*250) + (i5*1250); % new state after moving left
      [r]=R(r_state,Rg,Ro,Rj); % immediate reward
      % reward for possible result states
      sub_reward = 0;
      for rd=0:4
        for jk=0:4
          sub_state = i4 + (action*5) + (i5*50) + (rd*250) + (jk*1250);
          % sub_reward .+= V_star(sub_state+1);
          % [sub_state_reward] = R(sub_state,Rg,Ro,Rj);
          sub_state_reward = (1/25) .* V_star(sub_state+1);
          sub_reward .+= sub_state_reward;
        end;
      end;
      Q_table(s_i, 1) = r .+ gamma .* sub_reward;
      ...
      Q_table(s_i, 2) = r + gamma.*sub_reward;

      % action of moving right
      action = i2;
      if(i2 < 9) % when there is sapce to move, else stay at left-most place
        action = i2 + 1;
      end;
      r_state = i1 + (action*5) + (i3*50) + (i4*250) + (i5*1250); % new state after moving left
      [r]=R(r_state,Rg,Ro,Rj); % immediate reward
      ...
      Q_table(s_i, 3) = r + gamma.*sub_reward;

      % update policy and vstar
      [v ind] = max(Q_table(s_i,:));
      V_star(s_i) = v;
      Policy(s_i) = ind;
      % fflush(stdout);
      % [V_star(s_i) Policy(s_i)]
    end; % end of one round
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% TO DO:
    %%
